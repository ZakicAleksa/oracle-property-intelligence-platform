...
export default function ContractorDetailPage() {
  ...
  const { data, isLoading, isError } = trpc.contractors.detail.useQuery({
    companyId: params.id,
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-6 py-10 text-sm">
      <div className="fade-in border border-line bg-paper-raised p-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-2xl font-semibold">
            {company?.name ?? "Unknown contractor"}
          </h1>
          {bbbProfile !== undefined && (
            <span
              className={`stamp shrink-0 ${ratingStamp(bbbProfile.bbbRating)}`}
            >
        ...
        {bbbProfile !== undefined && (
          <p className="mt-1 text-ink-muted">
            {bbbProfile.isAccredited === true
              ? "BBB accredited"
              : "Not BBB accredited"}{" "}
            &middot; {bbbProfile.reviewCount ?? 0} reviews &middot;{" "}
            {bbbProfile.complaintCount ?? 0} complaints
          </p>
        )}
        {bbbProfile === undefined && (
          <p className="mt-1 text-ink-muted">No BBB profile on file.</p>
        )}
      </div>

      <Section label={`BBB complaints (${complaints.length})`}>
        {complaints.length === 0 && (
          <p className="text-ink-muted">No complaints on file.</p>
        )}
            ...
            <li key={i} className="ledger-row px-1 py-2">
              <div className="flex items-center justify-between gap-3">
                <span className="font-medium">
                  {c.complaintType ?? "Unknown type"}
                </span>
                <span className="stamp stamp-open">
                  {c.complaintStatus ?? "unknown"}
                </span>
              </div>
              <p className="eyebrow mt-1">
                {c.complaintDate ?? "unknown date"}
              </p>
              {c.complaintSummary !== null && (
                <p className="mt-1">{c.complaintSummary}</p>
              )}
            </li>
          ))}
        </ul>
      ...
      <Section label={`Review summaries (${reviews.length})`}>
        {reviews.length === 0 && (
          <p className="text-ink-muted">No reviews on file.</p>
        )}
        <ul className="space-y-2">
          {reviews.map((r, i) => (
            <li key={i} className="ledger-row px-1 py-2">
              <div className="flex items-center justify-between gap-3">
                <span className="font-mono text-xs">
                  {r.reviewDate ?? "unknown date"}
                </span>
                <span className="stamp stamp-closed">
                  {r.reviewRating ?? "n/a"} / 5
                </span>
              </div>
