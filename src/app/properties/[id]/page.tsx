...
export default function PropertyDetailPage() {
  ...
  if (isError || data === undefined)
    return <div className="mx-auto max-w-3xl px-6 py-10 text-stamp-open">Couldn&apos;t load this property.</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-6 py-10 text-sm">
      <div className="fade-in border border-line bg-paper-raised p-5">
        ...
        <h1 className="text-2xl font-semibold">{property?.unnormalizedAddress ?? "Unknown address"}</h1>
      ...
      <Section label="Ownership history">
        {ownershipHistory.length === 0 && <p className="text-ink-muted">No ownership records.</p>}
        <ul className="space-y-2">
          {ownershipHistory.map((o) => (
            <li key={o.ownershipId} className="ledger-row flex items-center justify-between px-1 py-2">
        ...
        <ul className="space-y-2">
          {permits.map((p) => (
            <li key={p.propertyImprovementId} className="ledger-row px-1 py-2">
              ...
              <p className="mt-1">{p.projectDescription ?? p.improvementType ?? "No description on file"}</p>
              {p.contractorName !== null && <p className="text-ink-muted">Contractor: {p.contractorName}</p>}
            </li>
          ))}
        </ul>
      </Section>
      ...
      <Section label="Business occupancy / tenants">
        {occupancy.length === 0 && <p className="text-ink-muted">No occupancy records.</p>}
        <ul className="space-y-2">
          {occupancy.map((t) => (
            <li key={t.tenantId} className="ledger-row flex items-center justify-between px-1 py-2">
        ...
        <ul className="space-y-2">
          {projects.map((p) => (
            <li key={p.projectId} className="ledger-row px-1 py-2">
              ...
              {p.contractorName !== null && <p className="text-ink-muted">{p.contractorName}</p>}
            </li>
          ))}
        </ul>
      </Section>
