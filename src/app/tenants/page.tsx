...
export default function TenantsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      {isError && <p className="text-stamp-open">Couldn&apos;t load tenants.</p>}
          ...
          {data.map((row) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="eyebrow">
                  {row.unnormalizedAddress ?? "unknown address"} &middot; {row.cityName ?? "unknown city"}
                </p>
              </div>
              <span className="stamp stamp-neutral shrink-0">{row.occupancyStatus}</span>
            </Link>
          ))}
          {data.length === 0 && <p className="py-6 text-ink-muted">No tenant occupancy records derived yet.</p>}
        </div>
      )}
    </div>
  );
